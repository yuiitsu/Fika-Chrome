...
App.module.extend('common', function() {
    ...
    this.notification = function(text, type) {
        ...
        $('body').append(App.view.get_view('common', 'notification', {
            text: text,
            bg: bg
        }));
    ...
    /**
     * 获取指定格式时间
     * @param date 某一天的值，默认今天，1 为前一天，-1为后一天,或者为一个date对象
     * @param formate 时间格式 默认yyyy-mm-dd
     */
    this.getDate = function (date, formate) {
        date = date || 0;
        formate = formate || 'YYYY-MM-DD';
        formate = formate || '-';
        var d = date instanceof Date === true ? date : new Date(Date.now() - date * 24 * 60 * 60 * 1000),
            reg = /([Yy]*)([\\\/\-]?)([Mm]*)([\/\\\-]?)([Dd]*)\s?([Hh]*)?\:?([Mm]*)?\:?([Ss]*)?/g,
            match = reg.exec(formate),
            year = match && match[1] ? d.getFullYear() : '',
            fgf = match && match[2] ? match[2] : '-',
            month = match && match[3] ? d.getMonth() + 1 : '',
            day = match && match[5] ? d.getDate() : '',
            hour = match && match[6] ? d.getHours() : '',
            minutes = match && match[7] ? d.getMinutes() : '',
            seconds = match && match[8] ? d.getSeconds() : '';
        ...
        return year + (month ? fgf : '') + month + (day ? fgf : '') + day + (hour ? ' ' : '') + hour + (minutes ? ':' : '') + minutes + (seconds ? ':' : '') + seconds;
    };

    this.secFormatTime = function (mss) {
        var days = parseInt(mss / (60 * 60 * 24)),
            hours = parseInt((mss % (60 * 60 * 24)) / (60 * 60)),
            minutes = parseInt((mss % (60 * 60)) / (60)),
            seconds = parseInt(mss % 60);
        let d = days ?days + '天' : '';
    ...
    this.cajax = function(data, callback) {
        ...
        chrome.tabs.getCurrent(function(tab) {
            ...
            // 发送查询字发送到background
            chrome.extension.sendMessage(send_msg, function(res) {
                callback(res);
            });
        });
    };
